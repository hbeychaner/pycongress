...
from congress_sdk.data_collection.client import CDGClient
from congress_sdk.data_collection.id_utils import parse_url_to_id
from congress_sdk.models.people import Chamber, Member, Sponsor, SponsorRef
from congress_sdk.models.shared import (
    Activity,
    CountUrl,
    EntityBase,
    Format,
...
from congress_sdk.models.validators import convert_law_type, normalize_chamber
...
class Hearing(BaseModel):
    ...
    def build_id(self) -> str:
        """Build a deterministic hearing id.

        Preference order:
        1) `jacket_number` + `congress` (includes `part` when present)
        2) `library_of_congress_identifier` (prefixed)
        3) parsed URL fallback using `parse_url_to_id`
        4) ultimate fallback: record:... hash (handled by canonical_id)
        """
        # Prefer jacketNumber when present
        try:
            if self.congress and self.jacket_number:
                part_suffix = f":part{self.part}" if getattr(self, "part", None) else ""
                return f"hearing:{int(self.congress)}:{int(self.jacket_number)}{part_suffix}"
        except (TypeError, ValueError) as exc:
            logger.debug(
                "Failed to build hearing id from congress/jacket_number: %s", exc
            )

        # Use LOC identifier when available
        if getattr(self, "library_of_congress_identifier", None):
            return f"hearing:loc:{self.library_of_congress_identifier}"
        ...
        if url:
            try:
                ...
                logger.debug("Failed to parse URL to id in hearing build_id: %s", exc)

        # Let canonical_id fallback handle hashed id
        return ""
...
class CountryParty(BaseModel):
    """Country/party entry for treaties (simple name container)."""

    name: Annotated[str, Field(description="Country or party name.")]


class IndexTerm(BaseModel):
    """Index/subject term attached to a treaty."""

    name: Annotated[str, Field(description="Index term text.")]


class RelatedDoc(BaseModel):
    """Lightweight related-document entry referenced by a treaty."""

    title: Annotated[
        Optional[str], Field(default=None, description="Related doc title.")
    ]
    url: Annotated[
        Optional[HttpUrl], Field(default=None, description="Related doc URL.")
    ]
    doc_number: Annotated[
        Optional[str],
        Field(
            alias="docNumber", default=None, description="Document number if present."
        ),
    ] = None


class TreatyParts(BaseModel):
    """Container for parts metadata when a treaty is split into parts.

    The API returns an object; model as a flexible mapping to allow future fields.
    """

    # store as a raw dict of values when present
    raw: Annotated[Dict[str, Any], Field(default_factory=dict)]


class Treaty(BaseModel):
    """Treaty record with full typed fields mirroring the API envelope."""
    ...
    # Rich fields returned by the treaty endpoint
    actions: Annotated[
        Optional[CountUrl],
        Field(default=None, description="Count/url object for actions list."),
    ] = None
    congress_considered: Annotated[
        ...
        Field(
            ...
            default=None,
            description="Congress considered field when provided.",
        ),
    ] = None
    congress_received: Annotated[
        ...
        Field(
            ...
            default=None,
            description="Original congressReceived value from the API.",
        ),
    ] = None
    countries_parties: Annotated[
        List[CountryParty],
        Field(
            alias="countriesParties",
            default_factory=list,
            description="Which countries/parties are involved in the treaty.",
        ),
    ]
    in_force_date: Annotated[
        Optional[datetime],
        Field(
            alias="inForceDate",
            default=None,
            description="When the treaty enters into force.",
        ),
    ] = None
    index_terms: Annotated[
        List[IndexTerm],
        Field(
            alias="indexTerms",
            default_factory=list,
            description="Index terms for the treaty.",
        ),
    ]
    number: Annotated[
        Optional[int],
        Field(
            default=None,
            description="Numeric treaty number as returned by some API shapes.",
        ),
    ] = None
    old_number: Annotated[
        Optional[str],
        Field(
            alias="oldNumber",
            default=None,
            description="Legacy treaty number if present.",
        ),
    ] = None
    old_number_display_name: Annotated[
        Optional[str],
        Field(
            alias="oldNumberDisplayName",
            default=None,
            description="Legacy display name for old number.",
        ),
    ] = None
    parts: Annotated[
        Optional[TreatyParts],
        Field(
            default=None, description="Parts object when treaty is divided into parts."
        ),
    ] = None
    related_docs: Annotated[
        List[RelatedDoc],
        Field(
            alias="relatedDocs",
            default_factory=list,
            description="Related documents referenced by the treaty.",
        ),
    ]
    resolution_text: Annotated[
        Optional[str],
        Field(
            alias="resolutionText",
            default=None,
            description="Resolution text when present.",
        ),
    ] = None
    suffix: Annotated[
        Optional[str],
        Field(default=None, description="Suffix applied to treaty number/title."),
    ] = None
    titles: Annotated[
        List[Title],
        Field(default_factory=list, description="List of titles for the treaty."),
    ]
    topic: Annotated[
        Optional[str],
        Field(default=None, description="Topical classification for the treaty."),
    ] = None
    transmitted_date: Annotated[
        ...
        Field(
            ...
            default=None,
            description="Date the treaty was transmitted.",
        ),
    ] = None
    update_date: Annotated[
        Optional[datetime],
        Field(
            alias="updateDate",
            default=None,
            description="When the treaty entry was last updated.",
        ),
    ] = None

    @model_validator(mode="before")
    def _normalize_treaty_item(cls, values: dict):
        """Normalize API item shapes into the model's expected fields.

        The API may return `congressReceived` and `number` keys (or
        `treatyNumber`/`congress`); map those into `congress` and
        `treaty_number`. Also synthesize a `url` when missing using the
        API path pattern so downstream id parsing can work.
        """
        if not isinstance(values, dict):
            return values

        # Map congressReceived -> congress
        if "congressReceived" in values and "congress" not in values:
            try:
                values["congress"] = int(values.get("congressReceived"))
            except Exception:
                values["congress"] = values.get("congressReceived")

        # Map number -> treatyNumber/treaty_number
        if (
            "number" in values
            and "treatyNumber" not in values
            and "treaty_number" not in values
        ):
            values["treatyNumber"] = values.get("number")

        # Ensure treatyNumber is a string for validation
        if "treatyNumber" in values and values.get("treatyNumber") is not None:
            try:
                values["treatyNumber"] = str(values.get("treatyNumber"))
            except Exception as exc:
                logger.debug("Failed to coerce treatyNumber to str: %s", exc)

        # Synthesize url when missing and congress+number are available
        if "url" not in values or not values.get("url"):
            c = values.get("congress") or values.get("congressReceived")
            n = values.get("number") or values.get("treatyNumber")
            try:
                if c and n:
                    values["url"] = (
                        f"https://api.congress.gov/v3/treaty/{int(c)}/{int(n)}?format=json"
                    )
            except (TypeError, ValueError) as exc:
                logger.debug(
                    "Failed to synthesize treaty url from congress/number: %s", exc
                )

        return values

    def build_id(self) -> str:
        """Build a deterministic treaty id.

        Preference order:
        1) `congress` + `treaty_number` (alias `treatyNumber`)
        2) parsed URL fallback using `parse_url_to_id`
        3) ultimate fallback: return empty so `canonical_id` can hash
        """
        try:
            congress = getattr(self, "congress", None)
            tn = getattr(self, "treaty_number", None)
            if congress and tn:
                # treaty_number may include suffixes; normalize to str
                return f"treaty:{int(congress)}:{str(tn)}"
        except Exception:
            raise ValueError(
                "Could not build canonical id for Treaty: missing or invalid congress/treaty_number"
            )
...
class Amendment(BaseModel):
    ...
    links: Annotated[
        Optional[List[dict]],
        Field(
            alias="links", description="Related links associated with the amendment."
        ),
    ] = None
        ...
        try:
            ...
            if url:
                try:
                    ...
                    logger.exception(
                        "Failed to parse URL to id in Bill.build_id: %s", exc
                    )
            mapping = self.model_dump() if hasattr(self, "model_dump") else dict(self)
            return f"record:{abs(hash(str(mapping))) % (10**12)}"
        except Exception:
...
class BillAmendmentsResponse(BaseModel):
    ...
    pagination: Annotated[Optional[dict], Field(default=None)] = None
    request: Annotated[Optional[dict], Field(default=None)] = None
    amendments: Annotated[List[AmendmentMetadata], Field(default_factory=list)]
...
class BillCommitteesResponse(BaseModel):
    ...
    pagination: Annotated[Optional[dict], Field(default=None)] = None
    request: Annotated[Optional[dict], Field(default=None)] = None
    committees: Annotated[List[CommitteeMetadata], Field(default_factory=list)]
...
class BillCosponsorsResponse(BaseModel):
    ...
    pagination: Annotated[Optional[dict], Field(default=None)] = None
    request: Annotated[Optional[dict], Field(default=None)] = None
    cosponsors: Annotated[List[Sponsor], Field(default_factory=list)]
...
class BillRelatedBillsResponse(BaseModel):
    ...
    pagination: Annotated[Optional[dict], Field(default=None)] = None
    request: Annotated[Optional[dict], Field(default=None)] = None
    relatedBills: Annotated[
        List[BillMetadata], Field(default_factory=list, alias="relatedBills")
    ]
...
class BillSummariesResponse(BaseModel):
    ...
    pagination: Annotated[Optional[dict], Field(default=None)] = None
    request: Annotated[Optional[dict], Field(default=None)] = None
    summaries: Annotated[List[Summary], Field(default_factory=list)]
...
class BillTitlesResponse(BaseModel):
    ...
    pagination: Annotated[Optional[dict], Field(default=None)] = None
    request: Annotated[Optional[dict], Field(default=None)] = None
    titles: Annotated[List[Title], Field(default_factory=list)]
