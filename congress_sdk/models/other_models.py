...
from congress_sdk.models.shared import EntityBase, Format, CountUrl
from congress_sdk.models.bills import Committee
...
class HouseCommunication(EntityBase):
    ...
    communication_type: Annotated[
        ...
        Field(
            ...
            description="Parsed communication type (accepts communicationType alias).",
        ),
    ] = None
    ...
    def build_id(self) -> str:
        """Return canonical id for HouseCommunication in the requested format.

        Format: house-communication:{congress}:{communication_type.code}:{number}
        Falls back to parsing `url` or superclass behavior when parts missing.
        """
        # prefer explicit existing id
        ...
        if congress is not None and number is not None:
            ct = getattr(self, "communication_type", None)
            code = None
            if isinstance(ct, dict):
                code = ct.get("code") or ct.get("name")
            elif ct is not None:
                # pydantic model instance
                code = getattr(ct, "code", None) or getattr(ct, "name", None)

            if code:
                try:
                    return (
                        f"house-communication:{congress}:{str(code).lower()}:{number}"
                    )
                except Exception as exc:
                    logger.debug("Failed to build house-communication id: %s", exc)

        # fallback to url parsing when available
        if getattr(self, "url", None):
            from congress_sdk.data_collection.id_utils import parse_url_to_id

            return parse_url_to_id(str(self.url))
    ...
    def _normalize_house_communication(cls, values: dict):
        ...
        # Populate parsed `communication_type` from the raw `communicationType` envelope
        ct = values.get("communicationType")
        if ct and not values.get("communication_type"):
            values["communication_type"] = ct
...
class HouseRequirement(EntityBase):
    ...
    # Preserve additional fields returned in the `houseRequirement` envelope.
    active_record: Annotated[
        Optional[bool],
        Field(
            default=None,
            alias="activeRecord",
            description="Whether the requirement is active.",
        ),
    ] = None
    frequency: Annotated[
        Optional[str],
        Field(default=None, description="Reporting frequency.", alias="frequency"),
    ] = None
    legal_authority: Annotated[
        Optional[str],
        Field(
            default=None, alias="legalAuthority", description="Legal authority text."
        ),
    ] = None
    nature: Annotated[
        Optional[str],
        Field(default=None, alias="nature", description="Raw nature field from API."),
    ] = None
    number: Annotated[
        Optional[int],
        Field(default=None, alias="number", description="House requirement number."),
    ] = None
    parent_agency: Annotated[
        Optional[str],
        Field(default=None, alias="parentAgency", description="Parent agency name."),
    ] = None
    submitting_agency: Annotated[
        Optional[str],
        Field(
            default=None,
            alias="submittingAgency",
            description="Submitting agency name.",
        ),
    ] = None
    submitting_official: Annotated[
        Optional[str],
        Field(
            default=None,
            alias="submittingOfficial",
            description="Submitting official title.",
        ),
    ] = None
    update_date: Annotated[
        Optional[datetime],
        Field(
            default=None, alias="updateDate", description="When the record was updated."
        ),
    ] = None
    matching_communications: Annotated[
        Optional[CountUrl],
        Field(
            default=None,
            alias="matchingCommunications",
            description="CountUrl envelope for matching communications.",
        ),
    ] = None

    @model_validator(mode="before")
    def _populate_type_from_nature(cls, values: dict):
        # API uses `nature` for the requirement kind in some responses;
        # map it to `type` so our model always exposes `type`.
        if (not values.get("type")) and values.get("nature"):
            values["type"] = values.get("nature")

        # preserve camelCase aliases into our snake_case fields when present
        if values.get("updateDate") and not values.get("update_date"):
            values["update_date"] = values.get("updateDate")
        if values.get("activeRecord") and not values.get("active_record"):
            values["active_record"] = values.get("activeRecord")
        if values.get("legalAuthority") and not values.get("legal_authority"):
            values["legal_authority"] = values.get("legalAuthority")
        if values.get("parentAgency") and not values.get("parent_agency"):
            values["parent_agency"] = values.get("parentAgency")
        if values.get("submittingAgency") and not values.get("submitting_agency"):
            values["submitting_agency"] = values.get("submittingAgency")
        if values.get("submittingOfficial") and not values.get("submitting_official"):
            values["submitting_official"] = values.get("submittingOfficial")
        if values.get("matchingCommunications") and not values.get(
            "matching_communications"
        ):
            values["matching_communications"] = values.get("matchingCommunications")

        # If `updateDate` is present and `date` is missing, prefer updateDate
        if not values.get("date") and values.get("updateDate"):
            values["date"] = values.get("updateDate")

        return values

    def build_id(self) -> str:
        """Return a stable id for HouseRequirement records.

        Preferred form: `record:{number}` when `number` is present.
        Falls back to parsing the `url` when available, otherwise raises.
        """
        ...
        if getattr(self, "url", None):
            ...
            return parse_url_to_id(str(self.url))
        raise ValueError(
            "Could not build canonical id for HouseRequirement: no number or url present"
        )
...
class SenateCommunication(EntityBase):
    ...
    # Preserve raw API envelope fields commonly returned in `senateCommunication`
    abstract: Annotated[
        Optional[str],
        Field(
            default=None, description="Raw abstract text from the API", alias="abstract"
    ...
    @model_validator(mode="before")
    def _normalize_senate_communication(cls, values: dict):
        # Map communicationType object -> type string
        ct = values.get("communicationType")
        if (not values.get("type")) and isinstance(ct, dict):
            values["type"] = ct.get("name") or ct.get("code")

        # Populate parsed `communication_type` from raw envelope when present
        if ct and not values.get("communication_type"):
            values["communication_type"] = ct

        # Abstract -> subject.name (preserve abstract too)
        abstract = values.get("abstract")
        if abstract and not values.get("subject"):
            subj = {"name": abstract}
            if values.get("congressionalRecordDate"):
        ...
        # Copy aliases into snake_case preservation fields
        if values.get("congressionalRecordDate") and not values.get(
            "congressional_record_date"
        ):
            values["congressional_record_date"] = values.get("congressionalRecordDate")
        if values.get("sessionNumber") and not values.get("session_number"):
            try:
                values["session_number"] = int(values.get("sessionNumber"))
            except Exception:
                values["session_number"] = values.get("sessionNumber")
        if values.get("updateDate") and not values.get("update_date"):
            values["update_date"] = values.get("updateDate")
        ...
        # Default recipient to first committee name when present
        if not values.get("recipient"):
            comms = values.get("committees")
            if isinstance(comms, list) and comms:
                first = comms[0]
        ...
        # Coerce number which may come as a string
        if values.get("number") is not None and not isinstance(
            values.get("number"), int
        ):
            try:
                values["number"] = int(values.get("number"))
            except (TypeError, ValueError) as exc:
                logger.debug("Failed to coerce number to int: %s", exc)

        return values

    def build_id(self) -> str:
        """Return a stable id for SenateCommunication records.
        Preferred form: `senate-communication:{congress}:{communication_type.code}:{number}`
        Falls back to parsing the `url` or superclass behavior when parts missing.
        """
        # prefer explicit existing id
        if getattr(self, "id", None):
            return str(self.id)
        ...
        if congress is not None and number is not None:
            ...
            if code:
                try:
                    return (
                        f"senate-communication:{congress}:{str(code).lower()}:{number}"
                    )
                except Exception as exc:
                    logger.debug("Failed to build senate-communication id: %s", exc)
...
class NominationDetail(EntityBase):
    """Detailed nomination record (preserves full API envelope)."""

    # keep the full set of fields previously present on `Nomination` so
    # consumers that want the expanded shape can import `NominationDetail`.
    congress: Annotated[
    ...
    def _populate_common_fields(cls, values: dict):
        ...
        if not values.get("position") and values.get("nominees"):
            try:
                ...
                logger.debug(
                    "Failed to derive nomination position from nominees: %s", exc
                )
        ...
        if not values.get("nominee") and values.get("nominees"):
            try:
                ...
                logger.debug(
                    "Failed to derive nomination nominee from nominees: %s", exc
                )
...
class Nomination(EntityBase):
    """Minimal nomination record used by other endpoints (committee-meeting)."""

    congress: Annotated[Optional[int], Field(description="Congress number")] = None
    number: Annotated[Optional[int | str], Field(description="Nomination number")] = (
        None
    )
    citation: Annotated[Optional[str], Field(description="Official citation")] = None
    description: Annotated[
        Optional[str], Field(description="Nomination description")
    ] = None
    nomination_type: Annotated[
        Optional["NominationTypeInfo"], Field(default=None, alias="nominationType")
    ] = None
    ...
    url: Annotated[Optional[HttpUrl], Field(description="Item URL")] = None

    @model_validator(mode="before")
    def _normalize(cls, values: dict):
        # copy common camelCase aliases
        if values.get("nominationType") and not values.get("nomination_type"):
            values["nomination_type"] = values.get("nominationType")
        if values.get("latestAction") and not values.get("latest_action"):
            values["latest_action"] = values.get("latestAction")
...
class NominationListItem(EntityBase, RecordTypeBase):
    ...
    @model_validator(mode="before")
    def _infer_number_from_url(cls, values):
        try:
            if "number" not in values and values.get("url"):
                u = str(values.get("url"))
                if "/nomination/" in u:
                    tail = u.split("/nomination/")[-1].split("?")[0]
                    # pick last path segment as the nomination number when possible
                    seg = tail.split("/")[-1]
                    try:
                        values["number"] = int(seg)
                    except (TypeError, ValueError) as exc:
                        logger.debug(
                            "Nomination number segment not int, keeping raw: %s", exc
                        )
                        values["number"] = seg
        except Exception as exc:
            logger.debug("Failed to infer nomination number from url: %s", exc)
        return values
...
class TreatyListItem(EntityBase, RecordTypeBase):
    ...
    def build_id(self) -> str:
        """Return a deterministic id for list-level treaty entries.

        Uses `number` and `congress_received`/`congress_considered` when available.
        Format: treaty:{congress}:{number} (falls back to parsed URL)
        """
        try:
            num = getattr(self, "number", None)
            cong = getattr(self, "congress_received", None) or getattr(
                self, "congress_considered", None
            )
            if cong and num:
                return f"treaty:{int(cong)}:{int(num)}"
        except Exception:
            raise ValueError(
                "Failed to build treaty id from congress and number fields"
            )
        ...
        if getattr(self, "url", None):
            try:
                ...
                raise ValueError("Failed to parse treaty id from URL")
...
class MemberListItem(EntityBase, RecordTypeBase):
    ...
    @model_validator(mode="before")
    def _infer_bioguide_from_url(cls, values):
        try:
            if "bioguideId" not in values and values.get("url"):
                u = str(values.get("url"))
                if "/member/" in u:
                    tail = u.split("/member/")[-1].split("?")[0]
                    values["bioguideId"] = tail
        except Exception as exc:
            logger.debug("Failed to infer bioguideId from url in other_models: %s", exc)
        return values
