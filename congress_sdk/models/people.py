...
from congress_sdk.models.shared import CountUrl, EntityBase
...
class Member(EntityBase):
    ...
    def _infer_bioguide_from_url(cls, values):
        # If the incoming payload omitted the bioguide id but includes a
        # URL with the bioguide segment, derive it here so validation can
        # proceed without requiring caller-side heuristics.
        try:
            ...
            logger.exception("Failed to infer bioguideId from url: %s", exc)
            raise
        return values
    ...
    def _populate_common_fields(cls, model):
        ...
        try:
            ...
            logger.exception(
                "Failed to populate id in Member._populate_common_fields: %s", exc
            )
            raise
        ...
        try:
            ...
            logger.exception(
                "Failed to populate url in Member._populate_common_fields: %s", exc
            )
            raise
        ...
        try:
            ...
            logger.exception(
                "Failed to populate member_type in Member._populate_common_fields: %s",
                exc,
            )
            raise
        ...
        try:
            ...
            logger.exception(
                "Failed to populate state_code in Member._populate_common_fields: %s",
                exc,
            )
            raise
