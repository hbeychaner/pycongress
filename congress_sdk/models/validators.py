...
def convert_law_type(value: str):
    ...
    """
    from congress_sdk.models.bills import LawType
...
def parse_iso_datetime(value: str):
    ...
    for fmt in ("%Y-%m-%dT%H:%M:%SZ", "%Y-%m-%dT%H:%M:%S", "%Y-%m-%d"):
        try:
            ...
            # Try the next format; log at debug level to aid troubleshooting
            logger.debug("parse_iso_datetime: format %s failed: %s", fmt, exc)
            continue
