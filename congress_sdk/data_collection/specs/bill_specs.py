...
from congress_sdk.models.bills import (
    Bill,
    BillActionsResponse,
...
def _path_params_for_bill():
    """Return a standard ParamSpec list for bill path parameters.

    Produces three ParamSpec instances used across bill endpoints to map
    path segments (congress, bill type, bill number) from list records or
    URL segments.

    Returns:
        List[ParamSpec]: Param specifications for ``congress``, ``billType``,
        and ``billNumber``.
    """
    return [
        ParamSpec(
